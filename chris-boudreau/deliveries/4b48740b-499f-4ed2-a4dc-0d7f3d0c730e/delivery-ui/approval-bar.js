...
const REVIEW_SESSION_URL = 'https://app.reclaim.ai/m/florian-philippe/flexible-quick-meeting';
class DeliveryApproval extends HTMLElement {
  async connectedCallback() {
    ...
    this.terminalObserver = new IntersectionObserver(entries => {
      const visible = entries.some(entry => entry.isIntersecting);
      this.classList.toggle('terminal-visible', visible);
    }, {threshold: .18});
    const terminal = document.getElementById('delivery-review-end');
    if (terminal) this.terminalObserver.observe(terminal);
    try {
  ...
  disconnectedCallback() { this.observer?.disconnect(); this.terminalObserver?.disconnect(); window.removeEventListener('focus', this.onFocus); document.removeEventListener('click', this.onInteraction, true); }
  async api(path, body) {
  ...
  productLabel() {
    return this.dataset.product || 'LinkedIn Sprint';
  }
  pageLabel() {
  ...
  renderTerminal() {
    ...
    const state = this.data?.acceptance?.state;
    const approved = state === 'approved';
    const changed = state === 'changes_requested';
    target.innerHTML = `<style>
#delivery-review-end .delivery-decision{border-top:1px solid #cfccc3;padding-top:24px;max-width:720px}
#delivery-review-end .delivery-decision-status{margin:0 0 14px;color:#5a5cf9;font:600 12px/1.4 Poppins,Arial,sans-serif;letter-spacing:.08em;text-transform:uppercase}
#delivery-review-end .delivery-decision-actions{display:flex;flex-wrap:wrap;gap:10px}
#delivery-review-end button,#delivery-review-end a{display:inline-flex;align-items:center;justify-content:center;min-height:48px;padding:13px 19px;border:1px solid #9b988f;border-radius:0;background:transparent;color:#252522;font:600 13px/1.3 Poppins,Arial,sans-serif;text-decoration:none;cursor:pointer}
#delivery-review-end button.primary{background:#5a5cf9;border-color:#5a5cf9;color:#fff}
...
@media(max-width:620px){#delivery-review-end .delivery-decision-actions{flex-direction:column}#delivery-review-end button,#delivery-review-end a{width:100%}}
  ...
  async submit(state, note = '') {
    ...
    if (!this.data) { this.error = 'Open the private link in your delivery email to respond.'; this.render(); return; }
    this.saving = true;
  ...
  render() {
    ...
    const introFirst = d?.resources?.some(item => item.kind === 'sprint' && item.requiresAcceptance === true);
    ...
    else if (this.mode === 'confirm') decision = `<h2>Confirm your approval</h2><p>${e(d?.statement?.text || approveBody)}</p><label class="ack"><input id="ack" type="checkbox"> I have reviewed this ${introFirst ? 'Intro' : 'delivery'}.</label><button id="confirm" class="primary" disabled>Confirm approval</button><button id="cancel">Go back</button>`;
    else if (this.mode === 'changes') decision = '<h2>Send your feedback</h2><form id="feedback-form"><label for="feedback">Your feedback</label><textarea id="feedback" required maxlength="3000"></textarea><button class="primary" type="submit">Send feedback</button><button id="cancel" type="button">Go back</button></form>';
    else if (d) decision = `<h2>${e(approveTitle)}</h2><p>${e(approveBody)}</p><div class="actions"><button id="approve" class="primary">${introFirst ? 'Approve Intro' : 'Approve this delivery'}</button><button id="changes">Give feedback</button><a id="schedule" class="action" href="${e(REVIEW_SESSION_URL)}" target="_blank" rel="noopener">Schedule session</a></div>`;
    this.ui.innerHTML = `<style>
:host{display:block;position:relative;z-index:1001;font:13px/1.5 Poppins,Arial,sans-serif;color:#22232d}*{box-sizing:border-box}header{transition:opacity 220ms ease,transform 220ms ease}:host(.terminal-visible) header{opacity:0;transform:translateY(-110%);pointer-events:none}header{position:fixed;inset:0 0 auto;background:#fafafa;border-bottom:1px solid #dddde6;box-shadow:0 3px 14px #20203009}.preview{background:#262733;color:#f4f4fb;padding:5px 24px;font:11px/1.5 system-ui}.bar{display:flex;align-items:center;gap:28px;padding:14px 24px}.owner{margin-right:auto}.owner strong{font-size:14px}.owner small{display:block;font-size:11px;color:#61626f}nav{display:flex;gap:20px}a{color:#61626f;text-decoration:none;padding:9px 0;border-bottom:2px solid transparent}a[aria-current=page]{color:#22232d;border-color:#5a5cf9}.status{font-size:12px;white-space:nowrap}button{font:600 12px/1.4 Poppins,Arial,sans-serif;border:1px solid #b7b8c4;background:transparent;color:#22232d;padding:12px 16px;cursor:pointer;border-radius:0}.primary{color:#fafaff;background:#5a5cf9;border-color:#5a5cf9}button:disabled{opacity:.5;cursor:default}button:hover:not(:disabled){filter:brightness(.95)}a:focus-visible,button:focus-visible,input:focus-visible,textarea:focus-visible{outline:3px solid #5a5cf9;outline-offset:3px}.panel{position:fixed;top:var(--delivery-bar-height,100px);right:0;left:0;max-height:calc(100dvh - var(--delivery-bar-height,100px));overflow:auto;padding:28px max(24px,calc((100vw - 820px)/2));background:#fafafa;border-bottom:1px solid #dddde6;box-shadow:0 15px 30px #20203018}.panel[hidden]{display:none}h2{font:600 22px/1.3 Poppins,Arial,sans-serif;margin:0 0 14px}p{max-width:65ch;margin:0 0 20px}.actions{display:flex;flex-wrap:wrap;gap:10px;margin:0 0 20px}button+button{margin-left:10px}.actions button+button{margin-left:0}.panel a.action{display:inline-flex;align-items:center;justify-content:center;font:600 12px/1.4 Poppins,Arial,sans-serif;border:1px solid #b7b8c4;background:transparent;color:#22232d;padding:12px 16px;border-radius:0;text-decoration:none}.ack{display:flex;align-items:center;gap:10px;margin:22px 0}.ack input{width:18px;height:18px;accent-color:#5a5cf9}textarea{width:100%;min-height:100px;font:inherit;padding:12px;margin:8px 0 16px;border:1px solid #b7b8c4}.receipt{color:#61626f;font-size:12px}.feedback{white-space:pre-wrap}[role=alert]{color:#94252d;margin-top:15px}[role=alert]:empty{display:none}.close{display:block;margin:22px 0 0;border:0;padding:5px 0;text-decoration:underline;font-weight:400}.status:before{content:'';display:inline-block;background:#5a5cf9;width:6px;height:6px;border-radius:50%;margin-right:7px}
...
@media(max-width:760px){.bar{display:grid;grid-template-columns:1fr auto;gap:0 12px;padding:5px 14px;min-height:70px}.owner{grid-column:1;gap:8px}.owner strong{font-size:11px}.preview{font-size:8px}nav{grid-row:2;gap:16px}.status{grid-column:2;grid-row:1;font-size:10px}#respond{grid-column:2;grid-row:2;padding:6px 10px;min-height:30px}.panel{padding:24px 18px}.panel .actions{flex-direction:column}.panel button,.panel a.action{margin:0 0 10px;width:100%}.panel .close{text-align:left}h2{font-size:20px}}
</style><header><div class="bar"><div class="owner"><strong>${e(this.dataset.name || d?.brandName)}</strong>${this.dataset.previewSession ? '<span class="preview" title="Local preview. Test responses only.">Test preview</span>' : ''}<small>${e(product)}${d ? ' · Version ' + d.revision : ''}</small></div><nav aria-label="Your delivery">${nav}</nav><span class="status" role="status">${e(status)}</span><button id="respond" class="${state === 'pending' ? 'primary' : ''}" aria-expanded="${this.open}" aria-controls="panel">${this.open ? 'Close response' : action}</button></div></header><section id="panel" class="panel" aria-label="Delivery response" ${this.open ? '' : 'hidden'}>${decision}<p role="alert">${e(this.error)}</p><button id="close" class="close">Back to ${e(pageLabel)}</button></section>`;
